...
class Model_Trainer:
                    ...
                    while True:
                        try:
                            ...
                            if global_step==1 or global_step % self.config['summary_step']==0 or global_step==self.max_train_steps:
                                ...
                                if global_step % (100*self.config['summary_step']) == 0 or global_step == self.max_train_steps:
    ...
    def print_log(self, global_step, config, proc_duration, current_lr, loss_value, f1, auc):
        ...
        logging.info(format_str % (datetime.now(), global_step, self.max_train_steps, f1, auc, loss_value,
    ...
    def write_summaries(self, summary_writer, summary_str, global_step, curloss, curauc, curf1, is_train=True):
        ...
        if(is_train==True):
            ...
            #curf1 = np.mean(curf1)
            logging.info("%s: step %d/%d: train_loss = %.6f, train_auc = %.4f, train_f1 = %.4f" \
                  % (datetime.now(), global_step, self.max_train_steps,
                     curloss, curauc, curf1))
        else:
            logging.info("%s: step %d/%d: valid_loss = %.6f, valid_auc = %.4f, valid_f1 = %.4f" \
                  % (datetime.now(), global_step, self.max_train_steps,
                     curloss, curauc, curf1))
